...
function generateNewArray() {
  ...
  barsCon.innerHTML = ''
  if (n > 30) {
    document.documentElement.style.setProperty('--width', '30px')//if num of bars increases then reduce the size
  } else {
    document.documentElement.style.setProperty('--width', '40px')//size of bars
  }
  for (let i = 0; i < n; i++) {
    barsHeight[i] = randomNumber(100, 500)//for bars height
    bars[i] = document.createElement('div')//created using div
    bars[i].classList.add('bar')
    barsCon.appendChild(bars[i])
    bars[i].style.height = barsHeight[i] + 'px'
  }//just create a bar at a single time provide it a hight ad it into array until the value of n by the size drag bar value
  let i = Math.floor(Math.random() * n)
  barsHeight[i] = 500
  bars[i].style.height = barsHeight[i] + 'px' //height using func
}
